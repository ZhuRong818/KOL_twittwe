...
# 政治：更看重“政治相关性”而不是纯互动（避免情绪党/流量党）
W_SIM = 0.6
W_ENG = 0.4

# 相似度门槛：避免“偶尔谈政治”的账号混进来
MIN_SIM_POLITICS = 0.12

# 稳定输出门槛：最近 N 条里至少多少条出现“核心政治词”
MIN_CORE_HITS = 5

TOP_N_OUTPUT = 300
OUT_RANKED_CSV = "politics_full.csv"
OUT_TOP_TXT = "politics_top300.txt"
...
POLITICS_CORE = [
    ...
    "state", "regime", "war"
    "democracy", "authoritarian", "autocracy",
    "election", "vote", "voting", "ballot",
    "campaign", "candidate", "polls",
...
POLITICAL_SYSTEMS = [
    ...
    "rule of law", "judicial", "judiciary",
...
IDEOLOGY_PARTIES = [
    ...
    "progressive", "socialist", "libertarian",
    "populism", "nationalism",
    "party", "opposition", "coalition",
    "democrats", "republicans",
    "labour", "conservatives","salary"
]

PUBLIC_ISSUES = [
    "immigration", "border", "asylum",
    "climate policy", "energy policy",
    "healthcare policy", "education reform",
    "tax policy", "welfare", "social security",
    "inequality", "social justice",
    "housing crisis", "crime policy",
    "gun policy", "abortion policy" ,"abortion",
    "race", "gun", "gender", "femminism",
    "lgbt", "lgbtq", "gay", "trans", "transgender"

]

GEOPOLITICS_SECURITY = [
    "geopolitics", "foreign policy", "diplomacy",
    "sanctions", "treaty", "alliance",
    "nato", "un", "united nations",
    "security", "national security",
    "defense", "military", "war", "conflict",
    "ceasefire", "invasion", "Russia", "Ukraine",
    "Gaza", "Trump", "tariff", "Xi Jinping"
    "Greenland", "Taiwan", "venezuela",
    "NATO", "ASEAN", "EU", "BRICS"
]

POLITICAL_DISCOURSE = [
    "this administration", "the government",
    "policy failure", "policy success",
    "political consequences", "public opinion",
    "voters will", "mandate", "legitimacy",
    "accountability", "corruption", "scandal",
...
def compute_core_hit_ratio(texts: List[str], core_terms: List[str]) -> float:
    """
    计算最近 tweets 中，至少命中一个 core term 的 tweet 占比。
    用于“稳定输出政治内容”的过滤。
    """
    texts = [t for t in texts if t]
    if not texts:
        return 0.0
    core = [c.lower() for c in core_terms]
    hits = 0
    for tx in texts:
        low = tx.lower()
        if any(c in low for c in core):
            hits += 1
...
def main():
    ...
    for i, u in enumerate(handles, 1):
        ...
        if not eligible:
            temp.append(UserScore(
                ...
                sim_raw=0.0, eng_raw=0.0, spam_ratio=0.0, rt_ratio=0.0, core_hit_ratio=0.0,
        ...
        try:
            ...
            sim_raw = compute_similarity_avg(texts, POLITICS_KEYPHRASES)
            eng_raw = compute_engagement_raw(tweets, p.followers)
            spam_r, rt_r = compute_spam_rt(texts)
            core_hit_ratio = compute_core_hit_ratio(texts, POLITICS_CORE)

            # ✅ Politics 专用过滤：低相似度 / 不稳定政治输出
            err = ""
            if sim_raw < MIN_SIM_POLITICS:
                err = "low_political_relevance"
            else:
                # 至少 MIN_CORE_HITS 条命中 core terms
                core_hits = int(round(core_hit_ratio * max(len(texts), 1)))
                if core_hits < MIN_CORE_HITS:
                    err = "unstable_political_output"

            temp.append(UserScore(
                username=u, followers=p.followers, eligible=True,
                sim_raw=sim_raw, eng_raw=eng_raw,
                spam_ratio=spam_r, rt_ratio=rt_r, core_hit_ratio=core_hit_ratio,
                sim_norm=0.0, eng_norm=0.0, score=(0.0 if not err else -1e9),
                error=err
            ))
    ...
    for k, idx in enumerate(eligible_idx):
        ...
        # ✅ Politics：RT 惩罚更重（政治“纯转发号”很多）
        penalty = 0.25 * s.spam_ratio + 0.25 * s.rt_ratio
    ...
    with open(OUT_RANKED_CSV, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            ...
            "spam_ratio", "rt_ratio", "core_hit_ratio",
            "score", "error"
        ])
        for r, s in enumerate(ranked, 1):
            w.writerow([
                ...
                f"{s.spam_ratio:.3f}", f"{s.rt_ratio:.3f}", f"{s.core_hit_ratio:.3f}",
